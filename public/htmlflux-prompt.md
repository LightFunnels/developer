...
2. Use Tailwind CSS for layout/typography/colors. Use a `<style>` block for: gradients, complex animations, `lf-selected` / `lf-active` / `lf-unavailable` overrides, `lf-slot-*` / `lf-bundle-slot-*` / `lf-variant-option-*` styling, and anything Tailwind cannot express cleanly. When the user wants the block to **match the funnel's branding**, use the funnel style variables (`var(--static_*, fallback)` — see "Funnel style variables") instead of hard-coded colors/fonts.
...
4. **Interactivity, in this order**:
   1. `data-lf-*` attributes for anything the runtime already handles (clicks tied to cart/variant/bundle/popup state, conditionals, repeaters, two-way inputs, redirects).
   2. **CSS-first** for visual interactions: scrollable galleries (`overflow-x-auto` + `scroll-snap-*`), hover/focus states, `<details>` for accordions, CSS animations.
   3. **Inline event handlers** (`onclick="..."`, `onmouseenter="..."`, etc.) when (1) and (2) don't cover it — fine for swapping the active thumbnail, scrolling to a slide, toggling a class.
   4. **Small `<script>` blocks** as a last resort for things like autoplay carousels, intersection-observer animations, video lightboxes. Two rules: wrap in an IIFE (`(function(){ ... })();`) so identifiers don't leak across HTMLFlux blocks, and scope all DOM queries under your wrapper class (e.g. `document.querySelector('.qmd .qmd-carousel')`). Scripts execute once on initial page load via SSR — they will NOT re-run when cart state changes, so don't use them to react to LF state.
   5. Still forbidden anywhere: fetching external data, cookies / localStorage / sessionStorage, third-party SDKs, analytics, navigation API.
...
1. `data-lf-step-uid="step_abc123"` — exact step match by UID. **Use this whenever the funnel has more than one step of the same type.**
2. `data-lf-page="checkout"` — resolves to the **first** step of that type in funnel order. Fine for single-product funnels with one step per type; ambiguous (you can't choose *which*) when there are several.
3. `data-lf-href="https://…"` — external URL fallback.

**Priority is presence-based, and there is NO fall-through on a miss.** The resolver commits to the highest-priority attribute that is *present* on the element — it does not try the next one if that attribute fails to resolve. So `data-lf-step-uid="step_typo"` (a UID that matches no step) returns nothing and the click **silently no-ops**; it will NOT fall back to `data-lf-page` / `data-lf-href`. Put exactly one targeting attribute on the element and make sure it resolves. (Inside a repeater, the UID from `$item` must use the dynamic-attr form: `data-lf-bind-attr-lf-step-uid="$item.uid"`.)
...
**The runtime enforces this** — for any other path (including every `$item.*` field; review text is customer-supplied) the value renders as escaped **plain text**, client-side and in SSR. So `data-lf-bind-html` on a non-trusted path isn't an XSS vector anymore, but it *will* show literal tags to the customer. Use `data-lf-bind` for everything that isn't one of the seven fields.
...
| `data-lf-bind-html="path"` | Sets `innerHTML` | Trusted fields only (description, policies) — **runtime-enforced**: any other path renders as escaped plain text. See gotcha #6. |
...
| `data-lf-bind-attr-{name}="path"` | Sets `data-{name}` from path | Only inside repeaters. For action-target attrs the name must start with `lf-` (so the result is a `data-lf-*` attr the click handler reads); never start it with `data-` (yields `data-data-…`). The runtime appends any suffix verbatim, so the `lf-` rule is about hitting a real target attr, not a hard constraint. |
| `data-lf-filter="currency"` | Formats numeric value as currency | Pair with `data-lf-bind` on prices. (`data-lf-filter="currency-no-group"` formats without thousands separators.) |
...
| `data-lf-if="path!=value"` | Same as `path=value!` — inequality, the form authors naturally write |

Falsy values: `false`, `0`, `null`, `undefined`, `""`, `[]`.

URL safety (`data-lf-bind-src` / `data-lf-bind-href` / `data-lf-href`): scheme **allowlist** — relative URLs plus `http:` / `https:` / `mailto:` / `tel:` / `sms:`. Any other scheme (`javascript:`, `data:`, `blob:`, `whatsapp:`, …) is silently dropped.
...
| `data-lf-variant-pick` | Option-picker element. Click changes the variant; element gets `lf-active` (and `lf-unavailable` when out of stock). Place it inside a `product.options` repeater (see snippet #3) — it isn't a top-level scanned attribute, so a block whose *only* `data-lf-*` attribute is `data-lf-variant-pick` is skipped entirely. |
...
| `select-variant` | Make a variant the active selection | `data-lf-variant-id` (must be a real numeric variant id from `product.variants`; an unbound/unknown/non-numeric id silently no-ops) |
| `select-bundle` | Activate a price bundle | `data-lf-bundle-id` (re-clicking the already-active bundle is a deliberate no-op, to preserve per-slot variant choices) |
| `close-cart` | Close the cart sidebar | — |
| `toggleCartPopup` | Toggle cart sidebar | — |
| `openPopup` / `togglePopup` | Open / toggle a popup | `data-lf-target` (popup id) |
| `closePopup` | Close the open popup | — (ignores `data-lf-target`; always closes whichever popup is active) |
| `scroll` | Smooth-scroll to a **native LF component** | `data-lf-target` = the **Anchor/ID** of a native LF Section (matched via the `data-id` LF injects), NOT a CSS selector and NOT an `id` on your own HTMLFlux markup. To scroll within/between HTMLFlux blocks use a native `<a href="#id">` instead — see "Cross-section anchors". |
| `redirect` | Navigate | `data-lf-step-uid` OR `data-lf-page` OR `data-lf-href` (priority order). Optional `data-lf-target` opens the destination in a new tab. |
...
Any input bound to a `.country` path is auto-replaced with a `<select>` dropdown. **It's only populated when the page also has a native LF address/checkout component** — that's what loads the country list. On a page whose only country field is HTMLFlux, the select renders empty and unselectable, so use the native LF checkout/address component for country selection. (The swap also fires on *any* path ending in `.country`, even a typo'd one not in the writable list — producing a dead, non-functional dropdown.)

Checkboxes bind through their **checked** state and dispatch `true`/`false` (not the `value` attribute) — use one for `collectLead.accepts_marketing`:

```html
<label><input type="checkbox" data-lf-input="collectLead.accepts_marketing" /> Email me offers</label>
```
...
| `lf-unavailable` | All variants matching this option value are out of stock | Manual `[data-lf-variant-pick]`, option elements rendered inside a `product.options` repeater, AND auto-generated `data-lf-bundle-slots` / `data-lf-variant-options` swatches (default style: opacity .4, diagonal stripe, click disabled). |

### Funnel style variables (CSS custom properties)

The funnel's Design settings are published on the page root as CSS custom properties — custom HTML inherits them. Use `var(--key, fallback)` to bind a block to the merchant's branding; there is NO `data-lf-bind` path for styles (CSS-only).

| CSS variable | Design setting | Type |
|---|---|---|
| `--static_page_background` | Page Background | color |
| `--static_section_background` | Section Background | color |
| `--static_title_text` | Title Text | color |
| `--static_body_text` | Body Text | color |
| `--static_primary_color_buttons_links` | Primary Color (Buttons & Links) | color |
| `--static_light_primary_background` | Light Primary Background | color |
| `--static_buttons_text` | Buttons Text | color |
| `--static_secondary_color` | Secondary Color | color |
| `--static_outline` | Outline | color |
| `--static_primary_links_hover` | Primary links hover | color |
| `--static_secondary_links_hover` | Secondary links hover | color |
| `--static_divider_color` | Divider color | color |
| `--static_heading_font` / `--static_subheading_font` / `--static_body_text_font` | Fonts | font-family (bare name, e.g. `Inter`) |
| `--static_heading_size` / `--static_subheading_size` / `--static_body_text_size` | Sizes | font-size (e.g. `54px`) |

Merchant-defined custom variables also land as `--<key>`.

Rules:
- **Always include a fallback** — `var(--static_outline, #d6dce9)`. The fullscreen editor preview is an isolated iframe without funnel vars; fallbacks are what render there. Canvas, SSR, and the published page get real values.
- Tailwind arbitrary values work: `bg-[var(--static_primary_color_buttons_links)]`.
- Fonts resolve to the bare family name — pair with a generic: `font-family: var(--static_heading_font, sans-serif)`.
- Use them when the user asks for "on-brand" / "match my funnel design"; hard-code only deliberately fixed palettes (e.g. an always-dark hero).
...
`product.*`, `checkout.*`, `order.*`, `state.cart.*`, `discount.*`, `collections.*`, `collectionList.*`, `signup.*`, `store.*`, `translation.*`, `lists.*`, `variantsList.*`, `priceBundlesList.*`, `shipping_zones.*`, plus the specific `account.*` paths in the data dictionary below (`account.name`, `account.currency`, `account.customers_email`, `account.address`, and the three `account.*_policy` / `terms_of_services` fields — `account.` is NOT a blanket prefix, so `account.foo` is rejected). Anything else returns undefined silently.
...
**Client-only — absent from the server-rendered HTML.** `_`-fields are computed during client-side hydration, not in the SSR pass. So on first paint (and for no-JS crawlers) every `_`-field is empty: bundle prices (`$item._total`, `$item._savings`) render blank, and any container gated on `data-lf-if="$item._selected"` — including the **required** `data-lf-bundle-slots` — is server-rendered hidden until hydration fills it in. Usually a sub-second flash for real users, but don't rely on `_`-fields (or the slot picker) being present in the initial HTML.
...
<style>
  ...
  /* Auto-generated swatches get .lf-unavailable when all matching variants are
     out of stock (default: opacity .4 + diagonal stripe + click disabled) —
     override [data-lf-variant-options] .lf-unavailable to restyle. */
</style>
```
...
<style>
  ...
  /* (auto-generated bundle slots apply .lf-unavailable to out-of-stock options — opacity .4, stripe, click disabled) */
</style>
```
...
### 15. Scrollable image gallery with thumbnail switcher

Pattern for product-page hero galleries: a horizontally scrollable strip of full-size slides + thumbnails that jump to the matching slide. Pure CSS scroll-snap does the layout; a tiny IIFE-wrapped script wires the thumbnails.

```html
<div class="qmd-gallery">
  <!-- Slides -->
  <div class="qmd-slides">
    <img data-lf-bind-src="product.images.0" alt="" class="qmd-slide" />
    <img data-lf-bind-src="product.images.1" alt="" loading="lazy" class="qmd-slide" />
    <img data-lf-bind-src="product.images.2" alt="" loading="lazy" class="qmd-slide" />
    <img data-lf-bind-src="product.images.3" alt="" loading="lazy" class="qmd-slide" />
  </div>

  <!-- Thumbnails -->
  <div class="qmd-thumbs">
    <button type="button" class="qmd-thumb qmd-active" data-qmd-i="0">
      <img data-lf-bind-src="product.images.0" alt="" loading="lazy" />
    </button>
    <button type="button" class="qmd-thumb" data-qmd-i="1">
      <img data-lf-bind-src="product.images.1" alt="" loading="lazy" />
    </button>
    <button type="button" class="qmd-thumb" data-qmd-i="2">
      <img data-lf-bind-src="product.images.2" alt="" loading="lazy" />
    </button>
    <button type="button" class="qmd-thumb" data-qmd-i="3">
      <img data-lf-bind-src="product.images.3" alt="" loading="lazy" />
    </button>
  </div>
</div>

<style>
  .qmd-gallery { display: flex; flex-direction: column; gap: 12px; }
  ...
  .qmd-slides::-webkit-scrollbar { display: none; }
  .qmd-slide {
    flex: 0 0 100%;
    aspect-ratio: 1 / 1;
    object-fit: cover;
  ...
  .qmd-thumbs { display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px; }
  .qmd-thumb {
    aspect-ratio: 1 / 1;
    border-radius: 10px;
    overflow: hidden;
  ...
  .qmd-thumb img { width: 100%; height: 100%; object-fit: cover; display: block; }
  .qmd-thumb.qmd-active { border-color: #111; }
</style>

<script>
  (function () {
    // Scope every query under the wrapper class so other HTMLFlux blocks on
    // this page don't get touched. The page may have multiple galleries with
    // different prefixes — each script only sees its own.
    var root = document.querySelector('.qmd-gallery');
    if (!root) return;
    var slides = root.querySelector('.qmd-slides');
    var thumbs = root.querySelectorAll('.qmd-thumb');

    // Click a thumb → scroll the corresponding slide into view
    thumbs.forEach(function (btn) {
      btn.addEventListener('click', function () {
        var i = Number(btn.getAttribute('data-qmd-i')) || 0;
        slides.scrollTo({ left: i * slides.clientWidth, behavior: 'smooth' });
      });
    });

    // Scroll the slides → mark the matching thumb active
    slides.addEventListener('scroll', function () {
      var i = Math.round(slides.scrollLeft / slides.clientWidth);
      thumbs.forEach(function (b, idx) {
        b.classList.toggle('qmd-active', idx === i);
      });
    });
  })();
</script>
...
**Three rules every JS snippet must follow** (this snippet demonstrates all three):

1. **IIFE-wrap** every `<script>` — `(function(){ ... })();`. Without this, `var slides` leaks into `window` and the next HTMLFlux block on the page that names something `slides` overwrites it.
2. **Scope DOM queries** to your wrapper class — `document.querySelector('.qmd-gallery .qmd-slides')`, not `document.querySelector('.gallery .slides')`. Page-builder pages often have multiple HTMLFlux blocks with similar markup; a bare `.slides` selector grabs whichever block ended up first in the DOM.
3. **Don't try to react to LF state changes** from your script. Scripts run once during initial page parse, then never again. If you need to update the gallery when the customer switches variants, do it via `data-lf-*` (e.g. bind the slide `src` to `product.variants.*.image` inside a `data-lf-repeat` that re-renders on selection).

If you need real autoplay, add a `setInterval` inside the same IIFE that calls `slides.scrollBy({ left: slides.clientWidth, behavior: 'smooth' })` and snaps back to 0 when at the end.

### 16. Common micro-patterns
...
<!-- Smooth-scroll anchor — use a native <a href> to scroll within your own HTMLFlux markup
     (data-lf-action="scroll" only resolves native LF Sections, by their Anchor/ID). -->
<a href="#bundles" class="text-sm underline">See pricing</a>
<!-- ...and put id="bundles" on the target element; add `html { scroll-behavior: smooth }` once. -->
...
`data-lf-action="scroll"` only resolves **native LF components** — it looks them up by the `data-id` LF injects (i.e. a Section's **Anchor/ID** field), so it cannot target an arbitrary `id` or selector on your own HTMLFlux markup (those silently no-op). For scrolling within or between HTMLFlux blocks, use a plain anchor + global smooth-scroll:
...
**Case 3 — the user wants something this doc doesn't cover.** Country-based content. Time-based offers. Fetching external data. Analytics event sends. Cookies. localStorage. Server-side fetches.
...
**Custom JS interactivity is NOT Case 3.** Carousels, autoplay sliders, image gallery thumbnail-switching, hover-driven overlays, intersection-observer scroll animations, video lightboxes — all fair game via inline handlers + small IIFE-wrapped `<script>` blocks (see Output rule #4 and the gallery snippet).
...
- Use a `<script>` tag (or inline handler) to fetch external data, hit third-party APIs, load SDKs, send analytics events, or touch the network in any way.
- Touch cookies, localStorage, sessionStorage, or the navigation API.
- Write a `<script>` block whose top-level `let` / `const` / `function` names aren't prefixed (collide across HTMLFlux blocks). Always IIFE-wrap.
- Rely on a `<script>` to react to LF state changes (variant pick, bundle pick, cart updates). Scripts run once on initial page load; they don't re-execute on state change. Use `data-lf-*` for state-reactive work.
...
- [ ] Any `<script>` block is IIFE-wrapped, scopes its DOM queries under the wrapper class, doesn't react to LF state changes, and doesn't fetch / touch storage / load SDKs.
