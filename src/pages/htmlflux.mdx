...
**The runtime does NOT enforce this** — it injects the resolved value as raw `innerHTML` for *any* allowed path. So binding `data-lf-bind-html` to untrusted / customer-supplied content (review text, testimonials, any `$item.*` field) is an XSS vector. Restricting it to the seven pre-sanitized fields is your responsibility; for everything else use `data-lf-bind` (escaped via `textContent`).
...
| `data-lf-bind-html="path"` | Sets `innerHTML` | Trusted fields only (description, policies) — convention, NOT runtime-enforced. See gotcha #6. |
...
| `lf-unavailable` | All variants matching this option value are out of stock | Manual `[data-lf-variant-pick]` and option elements rendered inside a `product.options` repeater (default style: opacity .4, diagonal stripe, click disabled). **NOT applied to auto-generated `data-lf-bundle-slots` / `data-lf-variant-options` swatches** — those only ever toggle `lf-active`, so out-of-stock options inside an auto-picker stay clickable and unstyled. |
...
<style>
  ...
  /* Note: auto-generated [data-lf-variant-options] swatches never receive .lf-unavailable —
     out-of-stock options can't be styled or disabled in an auto-picker. */
</style>
```
...
<style>
  ...
  /* (auto-generated bundle slots never receive .lf-unavailable — out-of-stock options stay clickable here) */
</style>
```
